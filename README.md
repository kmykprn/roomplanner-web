...
最初から一覧に入っている「サンプル 1」「サンプル 2」の 3D モデル（`src/assets/furniture/*.glb`）は、すべて自前で作ったものです。
...
そこから購入があると、このアプリの運営者に紹介料が入ることがあります。**商品の価格は変わりません。**
取り込んだ商品画像は端末の中にだけ置き、サーバーには残しません。
