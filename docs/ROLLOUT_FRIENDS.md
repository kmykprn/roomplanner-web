...
5. ログインを求めるパネルが出る。「Google でログイン」を押す（iPhone では「Google へ移動しています…」と出て、
   **ページごと Google に遷移する**。ポップアップは開かない）
6. Google のログイン画面で、A で登録したアカウントを選ぶ
7. iPhone では**アプリに戻ってきたあと「ログインできました。写真を選び直してください」と出る**ので、
   「＋ 写真から3Dモデルを作成」でもう一度写真を選ぶと作成が始まる（「作ったモデル」に円が出る）。
   ページを読み直すため、選んでいた写真は持ち越せない。PC のブラウザではポップアップなので、
   戻ってきた時点で選んだ写真のまま作成が始まる
8. 「利用者ID」を開き、**ログイン後の uid が 2 と同じ**ことを見る

| 見ること | 期待 | 違ったら |
|---|---|---|
| 6 でログイン画面が出る | 出る | 出ない・すぐ閉じる → パネルに理由が出る。その文言を控える |
| 7 で戻ってきたあとの案内 | 「ログインできました。写真を選び直してください」 | 出ない・「ログインしています…」のまま → 下の「困ったとき」 |
