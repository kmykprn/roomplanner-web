...
2. 「家具」タブ → 「利用者ID」を開き、**ログイン前の uid を控える**（長押しでコピー）
3. 作成ボタンの下に「作成には Google ログインが必要です」と出ていることを見る
4. 「＋ 写真から家具を作る」（または「3D モデルとして作る」）を押す。**写真を選ぶ前に**ログインを求めるパネルが出る
5. 「Google でログイン」を押す（iPhone では「Google へ移動しています…」と出て、
   **ページごと Google に遷移する**。ポップアップは開かない。PC ではポップアップ）
6. Google のログイン画面で、A で登録したアカウントを選ぶ
7. 戻ってきたら（iPhone）／ポップアップが閉じたら（PC）「ログインできました。もう一度
   「＋ 写真から家具を作る」を押して写真を選んでください」と出る。ボタンを押して写真を 1 枚選ぶと
   作成が始まる（「作ったモデル」に円が出る）
...
| 7 の案内 | 「ログインできました。もう一度…押して写真を選んでください」 | 出ない・「ログインしています…」のまま → 下の「困ったとき」 |
...
2. 「＋ 写真から家具を作る」→ 写真を選ぶ → 「Google でログイン」→ D と同じアカウント
...
| 「ログインしています…」のまま 60 秒以上止まる | iPhone なら旧版のまま。タブを閉じて開き直す。60 秒でパネルに「応答がありません」と出て押し直せる |
| Google 側で `redirect_uri_mismatch` | OAuth クライアントの承認済みリダイレクト URI に `https://kmykprn.github.io/__/auth/handler` が無い |
| 戻ってきたのに「ログインできました」が出ない | `https://kmykprn.github.io/__/auth/handler` が 200 / `text/html` で返るか（kmykprn.github.io リポジトリの Pages） |
