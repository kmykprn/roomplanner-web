 ...
 *            その下に、時間のかかる 3D モデルの作成（約 4 分）への入口を小さく置く。
 ...
 *   3 段目 … 基本の家具（椅子・テーブル…）
 *
 * 3D 生成は約 4 分かかるので、**待たせる画面ではなく、待たせない画面**にする。
...
export function createModelPanel({ onPlaced }: ModelPanelOptions): HTMLElement {
  ...
  generate3dButton.textContent = '3D モデルとして作る（約 4 分）';
