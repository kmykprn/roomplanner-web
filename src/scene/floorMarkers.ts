 ...
 * 見本を床の上で回しておく角度。
 *
 * **4 本の脚が重ならずに見える角度にする。** 脚は正方形に並んでいるので、
 *
 *   0°  … 奥の脚 2 本が手前の脚の真後ろに隠れ、接地点が 2 つしか見えない
 *   45° … 正方形の対角線が視線と重なり、奥の脚 1 本が手前の脚に隠れる（3 本）
 *   30° … どの 2 本も重ならず、接地点が四隅に散る
 *
 * 接地点が四隅に見えていれば、床に乗っているかどうかが読める
 */
const SAMPLE_TURN = (30 * Math.PI) / 180;
